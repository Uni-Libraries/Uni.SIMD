...
std::size_t PfbChannelizer_neon(const PfbChannelizerPlan& plan, PfbChannelizerState& state,
    ...
    const float* coefficients = PfbChannelizerAccess::coefficients(plan);
        ...
        if (decimation_phase == 0U) {
            if (selected_count != 0U) {
                ...
                for (std::size_t row = 0U; row < rows; ++row) {
                    ...
                    const float32x4_t samples_re0 = reverse_lanes(vld1q_f32(history_i + first_sample + 4U));
                    const float32x4_t samples_re1 = reverse_lanes(vld1q_f32(history_i + first_sample));
                    const float32x4_t samples_im0 = reverse_lanes(vld1q_f32(history_q + first_sample + 4U));
                    const float32x4_t samples_im1 = reverse_lanes(vld1q_f32(history_q + first_sample));
                ...
                const float32x4_t rotated_re0 =
                    vfmsq_f32(vmulq_f32(accumulator_re0, rotation_re0), accumulator_im0, rotation_im0);
                const float32x4_t rotated_re1 =
                    vfmsq_f32(vmulq_f32(accumulator_re1, rotation_re1), accumulator_im1, rotation_im1);
                const float32x4_t rotated_im0 =
                    vfmaq_f32(vmulq_f32(accumulator_im0, rotation_re0), accumulator_re0, rotation_im0);
                const float32x4_t rotated_im1 =
                    vfmaq_f32(vmulq_f32(accumulator_im1, rotation_re1), accumulator_re1, rotation_im1);
                ...
                pfb_inverse_fft_in_place(plan, fft_re.data(), fft_im.data());
