...
std::size_t PfbChannelizer_generic(const PfbChannelizerPlan& plan, PfbChannelizerState& state,
        ...
        if (decimation_phase == 0U) {
            if (selected_count != 0U) {
                for (std::size_t branch = 0U; branch < bins; ++branch) {
                    float accumulator_re = 0.0f;
                    float accumulator_im = 0.0f;
                    for (std::size_t row = 0U; row < rows; ++row) {
                        const float coefficient = coefficients[row * bins + branch];
                        const std::size_t sample_index = cursor + history_size - row * bins - branch;
                        accumulator_re = accumulator_re + history_i[sample_index] * coefficient;
                        accumulator_im = accumulator_im + history_q[sample_index] * coefficient;
                    }
